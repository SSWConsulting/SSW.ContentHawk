...
export function SseLog({ entries, className }: SseLogProps) {
  ...
  return (
    <div className={cn("text-xs bg-card border border-border rounded p-3 overflow-y-auto [scrollbar-color:theme(colors.border)_transparent] font-mono max-h-48", className)}>
      {entries.map((entry, i) =>
        entry.type === "link" ? (
