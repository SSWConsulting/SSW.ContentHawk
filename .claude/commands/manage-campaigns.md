...
You are preparing the arguments for `scripts/run-workflow.ts` in campaigns mode to do this you will read the state of all campaigns from markdown tables stored in the target repository.
...
Use the extracted Label as the key in `ContentCatalog`, and append the array of parsed `ContentItem`s as the value.
...
```bash
npx tsx scripts/run-workflow.ts campaigns <owner/repo> '<content-catalog-json>'
```
